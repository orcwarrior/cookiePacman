    ...
    class GameMap : engineReference
    {
       ...
       static public String MapsPath = "../data/Maps/";
        ...
        public GameMap(string bitPath,GameManager g)
        {
            ...
            background.layer = Layer._imgUnderTextBG;
            gameMgr = g;
        ...
        private void generateStaticMapToTexture()
        {
            ...
            //Delete old textures: NOTE, there will be none, delete them at Free
            ...
            // [TODO] This value should be depend on real window size (with borders)
            // not display resolution!!! [Best fix height property in Engine]
            // DONE!
            ...
            // NOTE: 2013-04-16 12:46:45 Before we Rendered static parts of
            //       Map by Viewport, now we do it in fly by this method.
            //       Hopeful, we get rid of all bugs.
            // ---
            ...
            for (int i = 0; i < staticTexCntY; i++)
            {
                ...
                for (int j = 0; j < Math.Ceiling(staticTexCntX/2.0); j++)
                {
                    ...
                    //arrayPointer.Offset(new Point(0, (arrayPointer.Y == 0) ? 0 : -1));
                    Point camShift = new Point((j * (objToRenderX - 1)) * GameManager.gridSize  +j * objShiftX,
                                               (staticTexSize*i) -32);// + i * objShiftY));
                    //                          (i * (objToRenderY - 1)) * GameManager.gridSize + i * objShiftY - 40);

                    if (i > 0) { camShift.Offset(0, -17); }

                    cam.SetPos(-camShift.X, -camShift.Y);
                    new DebugMsg("Moved camera to: (" + camShift.X + "," + camShift.Y + ")["+j+","+i+"]");
                    ...
                    for (int x = arrayPointer.X; (x < arrayPointer.X + objToRenderX && x < mapWidth); x++)
                    {
                        for (int y = arrayPointer.Y; (y < objToRenderY + arrayPointer.Y && y < mapHeight); y++)
                        {
                            if (mapRenderQueueFirst[x, y] != null)
                            {
                                ...
                                mapRenderQueueFirst[x, y].Render(-camShift.X, -camShift.Y);
                                //mapRenderQueueFirst[x, y].Free();
                            }
                        }
                    } for (int x = arrayPointer.X; (x < arrayPointer.X + objToRenderX && x < mapWidth); x++)
                    {
                        for (int y = arrayPointer.Y; (y < objToRenderY + arrayPointer.Y && y < mapHeight); y++)
                        {
                            if (paths[x, y] != null)
                            {
                                ...
                                paths[x, y].Render(-camShift.X, -camShift.Y);
                                //paths[x, y].Free();
                            }
                        }
                    } for (int x = arrayPointer.X; (x < arrayPointer.X + objToRenderX && x < mapWidth); x++)
                    {
                        for (int y = arrayPointer.Y; (y < objToRenderY + arrayPointer.Y && y < mapHeight); y++)
                        {
                            if (mapRenderQueue[x, y] != null)
                            {
                                ...
                                mapRenderQueue[x, y].Render(-camShift.X, -camShift.Y);
                                //mapRenderQueue[x, y].Free();
                            }
                        }
                    }
                    new DebugMsg("First rendered obj at: (" + (arrayPointer.X) * 48 + "," + (arrayPointer.Y) * 48 + "); - camshift");
                    new DebugMsg("Last rendered obj at: (" + (arrayPointer.X + objToRenderX) * 48 + "," + (objToRenderY + arrayPointer.Y) * 48 + "); - camshift");
                    //GL.Clear(ClearBufferMask.ColorBufferBit | ClearBufferMask.DepthBufferBit);
                    //Viewport.Render();
                    //engine.gameViewport.Render();

                    //engine.SwapBuffers(); //Make Front->Back cause we reading from back buffer
                    ...
                    for (int _y = 0; (_y < 1 && _y + i < staticTexCntY); _y++)
                    {
                        ...
                        { // 'magic' happens here:
                          ...
                          new DebugMsg("Next texture #" + idx + "; started at = ("+(_x * (staticTexSize-2)) +","+ (realYstartPos - staticTexSize) + ");", DebugLVL.info);

                          //GL.CopyTexSubImage2D(TextureTarget.Texture2D, 0, 0, 0, (_x * staticTexSize), realYstartPos - staticTexSize, staticTexSize, staticTexSize);
                          GL.CopyTexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, (_x * (staticTexSize-2)), realYstartPos- staticTexSize - 32, staticTexSize, staticTexSize, 0);
                          GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Nearest);
                          GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, (int)TextureMagFilter.Nearest);
                         // GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapS, (int)TextureWrapMode.Clamp);
                         // GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)TextureWrapMode.Clamp);

                          int startX = (_x * staticTexSize), startY = staticTexSize;// realYstartPos - staticTexSize - 48;
                          int width,height;
                          width = height = -staticTexSize;
                          //GL.Color4(120, 120, 120, 120);
        ...
        private void _prepareRenderOfMapStaticPart(int _x,int _y)
        {
            ...
            for (int y = staticTexCntY-1; y >=0; y--)
            {
                ...
                { //'magic' happens here: 
                    ...
                    int texID = y * staticTexCntX + x;
                    if (_debugingStaticMaps)
                    {
                        new DebugMsg("Static Map#" + texID + " Rendering Offset: [X=" + xoff + ",Y=" + yoff + "] / " + realYstartPos, DebugLVL.info);
                    }
                    //int texID = 2;
                        staticMapParts[texID].x = xoff;
                        staticMapParts[texID].y = yoff;
                        staticMapParts[texID].height = -1 * (int)staticMapParts[texID].orginalHeight;
                    staticMapParts[texID].prepareRender(); 
                }
            }
            _debugingStaticMaps = false;
        ...
        public void renderBackground()
        {
            ...
            background.OverrideTexcoords((0 - camOffsetFix.X / x), (1.8f - camOffsetFix.X / x),
                                         (0 + camOffsetFix.Y / y), (1.8f + camOffsetFix.Y / y));
        ...
        {            
            ...
            Camera c = new Camera(Camera.eType.STATIC);//engine.gameCamera;
